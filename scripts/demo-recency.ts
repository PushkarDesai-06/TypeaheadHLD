/**
 * Recency-ranking demonstration (Assignment §7 — "demonstrate the difference
 * between the two ranking approaches using sample data or logs").
 *
 * For one prefix it prints the suggestion list under BOTH rankings, then drives
 * a burst of searches for a modestly-popular query sharing that prefix, waits
 * for the write→Postgres→cache-updater pipeline to settle, and re-prints both
 * lists. Expected result:
 *   • rank=basic   (all-time count) : essentially UNCHANGED — history dominates.
 *   • rank=recency (blended score)  : the bursted query CLIMBS to the top.
 *
...
async function main() {
  ...
  // Burst: many searches for the hot query (routes by hash(query); the updater
  // bridges it to the prefix's shard cache).
