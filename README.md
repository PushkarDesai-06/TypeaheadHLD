...
| Suggest              | `src/server.ts` `GET /suggest`         | `ZREVRANGE q:<prefix> 0 N-1` on the local shard → up to 10 suggestions sorted by count. The cache it reads is maintained by the updater (via the app node).                                                                                  |
| Cache debug          | `GET /cache/debug?prefix=<p>`          | LB hashes the prefix, proxies to the owning node, which reports `{ node, shard, status: hit/miss, cached }` — shows which cache node owns a prefix and whether it is cached there.                                                            |
